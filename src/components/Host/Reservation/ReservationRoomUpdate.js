...
function ReservationRoomUpdate({ rooms, roomIndex, roomTypes, roomMoved, show, handleClose }) {
    const pageSize = 5;
    let i = 1;
    ...
    const fetchData = async () => {
        setRoomChildren(rooms.filter((x) => x.id !== roomMoved.roomId));
        setFilterRooms(rooms.filter((x) => x.id !== roomMoved.roomId));
        return;
    };
    ...
    const handleRoomMoved = async (room) => {
        let dataRoom = {
            ...room,
        ...
        dataRoom.roomReservationProducts = roomMoved.roomReservationProducts;
        dispatch(HostUpdateReservationSlice.actions.roomMoved({ dataRoom: dataRoom, roomMovedId: roomMoved.id }));
        ...
        <>
            <Modal show={show} onHide={handleClose} dialogClassName="modal-70w">
                <Modal.Header closeButton>
                    <Modal.Title>Chuyển phòng: {roomMoved.roomName}</Modal.Title>
                </Modal.Header>
                <Modal.Body>
                                            ...
                                            (room, index) =>
                                                ...
                                                index < maxIndex && (
                                                    <tr key={room.name}>
                                                        <td>{++index}</td>
                                                        <td>{room.name}</td>
                                                        <td>{room.roomType ? room.roomType.name : 'Chưa thiết lập'}</td>
                                                        <td>
                                                            {room.roomType ? (
                                                                <>
                                                        ...
                                                        <td>
                                                            ...
                                                            {room.status === 0 ? (
                                                                <>
                                                                    <span className="badge badge-danger">Chưa dọn</span>
                                                                </>
                                                            ) : (
                                                                <>
                                                                    <span className="badge badge-success">Đã dọn</span>
                                                                </>
                                                            )}
                                                        </td>
                                                        <td>
...
export default ReservationRoomUpdate;
